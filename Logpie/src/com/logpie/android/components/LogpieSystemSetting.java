...
public class LogpieSystemSetting
{
	...
	private static final String TAG = LogpieSystemSetting.class
			.getName();
	...
	{
		mDataPlatform = DataPlatform
				.getInstance(context);
	}

	public synchronized static LogpieSystemSetting getInstance(
			Context context)
	{
		if (sSingletonSetting == null)
		{
			sSingletonSetting = new LogpieSystemSetting(
					context);
		}
	 ...
	 * @return
	 */
	public boolean setSystemSetting(
			final String key, final String value)
	{
		if (mSettingsMap.containsKey(key)
				&& TextUtils.equals(
						mSettingsMap.get(key),
						value))
		{
			// If the cache key,value already exist, then just return;
			return true;
		}
		boolean success = mDataPlatform
				.getKeyValueStorage().insert(
						DataLevel.SYSTEM_LVL,
						key, value);
		if (success)
		{
			...
			final Bundle settings)
	{
		...
		settings.putString(
				DataLevel.KEY_DATALEVEL,
				DataLevel.SYSTEM_LVL.toString());
		boolean success = mDataPlatform
				.getKeyValueStorage().insert(
						settings);
		if (success)
		...
		} else
		{
			LogpieLog
					.e(TAG,
							"Set the system settings from Bundle fail!");
		}
		return success;
	}
