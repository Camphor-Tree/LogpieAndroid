...
public class LogpieSystemSetting
{
    ...
    {
        mDataPlatform = DataPlatform.getInstance(context);
    }
    ...
    public boolean setSystemSetting(final String key, final String value)
    {
        ...
        boolean success = mDataPlatform.getKeyValueStorage().insert(DataLevel.SYSTEM_LVL, key,
                value);
        if (success)
        {
    ...
    public boolean setSystemSetting(final Bundle settings)
    {
        ...
        boolean success = mDataPlatform.getKeyValueStorage().insert(settings);
        if (success)
        {
            for (String key : keySet)
            {
    ...
    private void populateAllSystemSettings()
    {
        if (mSettingsMap == null)
        {
            mSettingsMap = mDataPlatform.getKeyValueStorage().queryAll(DataLevel.SYSTEM_LVL);
        }
    }
}
