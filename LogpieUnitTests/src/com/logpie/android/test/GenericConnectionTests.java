...
public class GenericConnectionTests extends AndroidTestCase {
	...
	//TODO: there's a bug. it is always success, because the main thread just get through 
	//and do not wait for other threads' result.
	//we should refactor this method to let the current wait until all the sub-threads finished
	...
	private void testRocketService()
	{
		...
		try {
			connection.setmRequestData(new JSONObject(testRocketData));
